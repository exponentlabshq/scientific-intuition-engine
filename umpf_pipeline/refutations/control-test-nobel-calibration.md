...
## Sixth follow-up (2026-08-31): a second, independent model reviews all 13 real cases — two more real gaps found, v3 shipped

An independent high-reasoning model (o4-mini, not this project's own reasoning) was given the real, full text — hypothesis, verification, and refutation reasoning — of all 13 real Nobel-linked entries now on the public leaderboard, and asked to critique the pipeline's own lens reasoning directly. 10 real calls (~$0.24 total), reviewed manually in full.

**Strongly convergent finding, not new in kind but now far better evidenced: Triviality was judged unsound on 10 of 10 cases where it refuted a real discovery.** Every single independent review, worded differently each time, converged on the same underlying diagnosis already named in this file: the lens treats "this claim's abstract shape sounds generic" as equivalent to "this claim's specific content is trivial," which is a category error — nearly every deep scientific law has a simple abstract form once known.

**A genuinely new, previously-unnoticed finding: Coherence is mostly sound on the 8 SURVIVES cases, but was judged unsound on 4 of the 5 Contested (REFUTED) cases** — Watson/Crick/Franklin, Hopfield, Coase, and Feynman all flagged as "equivocation" on a term naming the *literal same formal object* across domains (Hopfield's energy function IS the Ising Hamiltonian; Coase's "transaction costs" is one formal economic concept; "double helix" names one real molecular geometry), not a genuinely different meaning. The existing v2 fix covered the *different-mechanism-same-structure* case (Nash); it never explicitly addressed the stronger, *literally-identical-object* case, and the lens was still failing on it.

**Two real, targeted fixes shipped as v3, `LENS_VERSION = "2026-08-31-v3"`:**
- **Coherence**: an explicit addendum — if the claim states the exact same equation, theorem, or physical structure (not an analogous one), take that at face value as literal identity, the strongest possible form of coherence, not a claim to second-guess. Named worked examples: Hopfield, Watson/Crick/Franklin, Coase, Feynman.
- **Triviality**: a new Step 5, additive to the existing Steps 1-4 (which stop paraphrase-before-testing) — ask whether an expert in either source domain, *before* the connection was established, would already have treated it as obvious or assumed. If establishing it required real, non-obvious work (a new derivation, a real experiment, overturning a prior consensus), it is not trivial regardless of how generic its summary sounds. Named worked examples: Hayek, Coase, Feynman.

**Fresh held-out test, since all 13 existing cases were now diagnostic material for v3, not held-out:** 5 genuinely new, real, well-documented Nobel-linked discoveries, none sourced from `bisociation_gold_pairs.json`, selected purely for real historical rigor — McClintock (transposons, 1983 Nobel Medicine), Marshall & Warren (H. pylori/ulcers, 2005 Nobel Medicine), Mitchell (chemiosmotic ATP synthesis, 1978 Nobel Chemistry), Prusiner (prions, 1997 Nobel Medicine), and Alvarez (K-T asteroid extinction, disclosed looser Nobel linkage — his 1968 Physics Nobel was for particle-physics work, not the extinction hypothesis itself, matching how Planck/Feynman/Einstein's own looser cases were handled).

**Result: 5 of 5 survived.** Coherence: 5-of-5 SURVIVES, clean. Testability: 5-of-5 SURVIVES, clean. Triviality: only 1-of-5 SURVIVES (Prusiner) — **the net result improved because Coherence and Testability got more reliable, not because Triviality was actually fixed.** Read plainly, not smoothed over: the 2-of-3 promotion rule is correctly finding the right answer on all 5 fresh cases, but it is doing so by outvoting a still-broken third lens, not by correcting it.

**Reading the real Triviality reasoning on the 4 REFUTED cases surfaced something more specific than "still miscalibrated":**
- **McClintock and Marshall/Warren are confirmed recurrences of the reasoning/verdict mismatch bug**, believed fixed earlier the same day. Both cases' own Triviality reasoning explicitly concludes the claim is *not* trivial ("is not a trivial statement... does not apply broadly across different domains"; "indicates that this was a novel and non-obvious discovery, not something that would be assumed or trivial") — and the verdict field says REFUTED anyway. The shared "set the verdict from your own reasoning" instruction added earlier today is not holding reliably.
- **Mitchell is a genuine recurrence of the original paraphrase-before-refuting pattern** Steps 1-4 were built to stop — the tested phrase ("cells...pump protons across a membrane to build up a...gradient") drops the claim's actual load-bearing specificity (that this gradient specifically drives ATP synthase, a named enzyme acting as a molecular turbine).
- **Alvarez is the one case that reads as a genuine, defensible disagreement** rather than an obvious bug — arguable, not clearly wrong.

**Loophole check, re-run with v3 active:** the genuinely speculative Identity Compression hypothesis still correctly refuted 0-of-3. The fixes are not a rubber stamp.

**Honest bottom line:** v3 is real, targeted progress on Coherence (now clean on all 5 fresh cases, closing the literal-identity gap) and represents a real, evidenced attempt at Triviality's deeper problem — but the fresh test shows Triviality itself is not yet reliably fixed, and surfaced a concrete, confirmed recurrence of a bug thought closed. Not chased further in this same pass, per this project's own standing discipline against tuning a rubric repeatedly against the same failure mode in one sitting — flagged as a real, well-diagnosed candidate for a future, separate, targeted fix (specifically the reasoning/verdict consistency instruction, which now has two independent confirmed failures behind it, not zero).

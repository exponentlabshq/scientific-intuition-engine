...
- **Phase 3 (real researcher outreach) — closed for real, in parallel, the same day.** Flagged by Rocky Nguyen's and Brian Ahuja's reviews as the actual gap that mattered most. Corrected here rather than left stale: a separate Cursor session working this same repo the same evening sent 3 sharpened outreach emails to real researchers (Igor Aronson, Erwin Frey, John B. Phillips), each pitching a real ADJACENT_ACTIVE hypothesis with an operational hard claim rather than a soft, near-known framing. A 21-day reply clock is now running. This section originally reported this as still open; it was corrected within hours of being written, which is itself worth naming as an instance of the same discipline this whole postmortem is about — check the real state before trusting an earlier report of it.

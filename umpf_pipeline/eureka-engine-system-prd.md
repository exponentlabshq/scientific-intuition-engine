...
- **Phase 3 sent for real, 2026-08-29 — no longer a limitation, now a live clock.** Corrected from an earlier draft of this section written before the sends: 3 sharpened outreach emails went out to real researchers (Igor Aronson/Penn State, Erwin Frey/LMU Munich, John B. Phillips/Virginia Tech), each pitching a real ADJACENT_ACTIVE hypothesis with an operational hard claim. A 21-day reply clock is running, due ~2026-09-19. Sending remains a human decision every time — nothing sends automatically — but the mechanism has now actually been exercised, not just built. Pass condition: ≥1 `researcher_confirmed_novel` or ≥2 `researcher_confirmed_known`, logged to the ledger's `outreach_status` field when replies land. Three more sharpened drafts are ready and not yet sent.
- **COA 2d send-ready gate (2026-08-29):** before any Phase 3 send, sharpen the hypothesis to a closed-loop hard claim, re-verify soft vs hard (soft near-known neighbors do not earn the packet), and require `packets_manifest.json` `sharpened: true`. Soft-trap exemplar: Game Theory × Music → DiCola COLLISION after sharpen. All 3 sent emails passed this gate. See `outreach/README.md` and `sharpen_hypothesis.py`.

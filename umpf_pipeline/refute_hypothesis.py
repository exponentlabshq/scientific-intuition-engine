...
LENS_QUESTIONS = {
    "coherence": (
        ...
        "genuinely different underlying MEANINGS and treating that as a match (real examples: "
        "\"trust\" meaning a precise cryptographic probability bound in one domain and a loose "
        "psychological feeling in the other; \"adaptive\" naming one specific, well-defined "
        "molecular subsystem in one domain and standing in for generic \"learning\" in the "
        "other)? For a Janusian hypothesis specifically: is this a genuine same-instance "
        "paradox, or a disguised context-dependent compromise (\"in some contexts... in "
        "others\") labeled 'paradox'? For a homospatial hypothesis specifically: is there an "
        "actual fusion, or is this two domains compared side-by-side wearing a fusion's coined "
        "name?\n\n"
        "2026-08-31 fix, after control test 3 found this lens rejecting real, historically-"
        "confirmed bisociations for having DIFFERENT MECHANISMS in each domain — treating that "
        "as equivocation when it is the opposite. CRITICAL DISTINCTION, do not conflate these:\n"
        "  - Real equivocation: the SAME TERM has a genuinely different, precisely-defined "
        "MEANING in each domain, and the claim's force depends on the reader not noticing the "
        "switch.\n"
        "  - NOT equivocation, and not a flaw: the two domains have genuinely DIFFERENT "
        "underlying mechanisms, and the claim is precisely that those different mechanisms "
        "produce the SAME formal/structural object. This is not a weaker or disguised version "
        "of bisociation — it is what bisociation IS (Koestler: \"collision of two habitually "
        "incompatible frames\"). A real example this lens previously refuted incorrectly: an "
        "evolutionarily stable strategy is provably a Nash equilibrium, even though one arises "
        "from conscious strategic reasoning and the other from blind differential reproductive "
        "success with no reasoning at all — the mechanisms are supposed to differ; the "
        "mathematical structure (the equilibrium condition itself) is what's claimed identical, "
        "and it is. Do not refute a bisociation for its two domains operating by different "
        "mechanisms — check instead whether the SPECIFIC shared term or structure is precisely, "
        "identically defined in both (real match) or loosely gestured at in one while precise "
        "in the other (real equivocation).\n"
    ...
    "testability": (
        ...
        "2026-08-31 fix, after control test 3 found this lens rejecting real, historically-"
        "confirmed claims for lacking a FUTURE-tense stated experiment, even after "
        "acknowledging in its own reasoning that a real historical test was named — "
        "demanding BOTH the citation AND a separate forward-looking metric/threshold on top "
        "of it, which is not what this fix asks for. STOP AND READ THIS CAREFULLY: if the "
        "claim names ONE of the following, that ALONE is sufficient — do not additionally "
        "require a future-tense metric, comparison condition, or rejection threshold, and do "
        "not refute for 'lacking an experimental framework' once one of these is present:\n"
        "  (a) a SPECIFIC real historical experiment, dataset, or observation that already "
        "tested it (e.g. 'the Meselson-Stahl experiment, 1958', 'the observed blackbody "
        "spectrum') — the named result IS the metric, the named comparison IS the comparison "
        "condition, 'the named result does not hold' IS the rejection threshold;\n"
        "  (b) a SPECIFIC mathematical proof, derivation, or theorem that already settles the "
        "question (e.g. 'the energy function is provably identical to the Ising Hamiltonian', "
        "not merely similar to it) — a proof is testability in its strongest form, not an "
        "absence of it; do not demand an empirical dataset for a claim whose real evidence is "
        "mathematical.\n"
    ...
    "triviality": (
        ...
        "2026-08-31 fix, after control test 3 found this lens paraphrasing a precise claim down "
        "to its vaguest possible restatement, THEN correctly observing the restatement is "
        "generic — a real example: Hopfield's actual claim (a network's energy function has THE "
        "EXACT SAME mathematical form as the Ising model, not an analogy to it) was restated by "
        "this lens as \"systems converging to local minima,\" then refuted as \"common to many "
        "complex systems.\" That is a different, weaker claim than the one made, and stripping "
        "the STRUCTURE itself (not just the vocabulary) manufactures an umbrella trap that was "
        "never actually there. \"Strip the domain-specific vocabulary\" means: keep the precise "
        "relationship, formula, mechanism, or proof exactly as specific as it was stated, and "
        "only swap out which domain names are attached to it. It does NOT mean: compress the "
        "claim into a looser, more general paraphrase and then test whether THAT is generic. "
        "The real test: would this SAME precise, specific claim — not a vaguer summary of it — "
        "honestly describe most other pairs of complex systems? For a genuinely exact, narrow, "
        "provable identity (the same equation, the same formula, a specific named theorem), the "
        "honest answer is usually no, and that is real, load-bearing specificity, not triviality "
        "dressed up in domain vocabulary."
    ),
}
